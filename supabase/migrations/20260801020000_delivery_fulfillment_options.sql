-- Modalidades configurables para el menu publico: domicilio, retiro o ambas.

begin;

alter table public.restaurants
  add column if not exists delivery_home_enabled boolean not null default true,
...
alter table public.orders add constraint orders_fulfillment_type_check
  check (fulfillment_type is null or fulfillment_type in ('home_delivery', 'pickup'));

update public.orders set fulfillment_type = 'home_delivery'
where order_type = 'delivery' and fulfillment_type is null;
...
drop function if exists public.create_delivery_order(text, jsonb, text, text, text, text, uuid, text);
...
grant execute on function public.create_delivery_order(text, jsonb, text, text, text, text, uuid, text, text)
