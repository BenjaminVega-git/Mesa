-- La geolocalización del navegador incluye un margen de error. Usarlo evita
-- rechazar a una persona que está en el local cuando el GPS ha dado una
-- posición ligeramente desplazada. El margen aportado por el cliente se limita
-- para no convertir la validación en una autorización ilimitada.
...
begin
  ...
  if coalesce(v_restaurant.enabled, false) then
    ...
    -- Como máximo se añade un radio extra igual al configurado. Así una
    -- lectura con error no bloquea al cliente, pero el margen nunca duplica
    -- indefinidamente el área permitida.
...
begin
  ...
  if coalesce(v_restaurant.enabled, false) then
    ...
    if v_distance_m > v_allowed_m then raise exception 'Estas demasiado lejos del local para enviar pedidos desde esta mesa'; end if;
  end if;
