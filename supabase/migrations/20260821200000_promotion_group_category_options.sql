-- Grupos de promo con opciones por multiples categorias.
-- Prioridad de opciones:
--   1) promotion_group_options: productos puntuales
--   2) promotion_group_category_options: categorias alternativas
--   3) promotion_groups.category_id: categoria base historica

create table if not exists public.promotion_group_category_options (
...
create index if not exists idx_promotion_group_category_options_group
  on public.promotion_group_category_options (group_id);
create index if not exists idx_promotion_group_category_options_category
  on public.promotion_group_category_options (category_id);

alter table public.promotion_group_category_options enable row level security;
revoke all on public.promotion_group_category_options from anon, authenticated;
  ...
  join public.products p
   ...
   and (
     ...
     or (
       ...
       and exists (select 1 from public.promotion_group_category_options gco where gco.group_id = g.id)
       and exists (
         select 1
         from public.promotion_group_category_options gco
         where gco.group_id = g.id
           and gco.category_id = p.category_id
       )
     )
     or (
       not exists (select 1 from public.promotion_group_options go where go.group_id = g.id)
       and not exists (select 1 from public.promotion_group_category_options gco where gco.group_id = g.id)
  ...
  select jsonb_build_object(
    ...
    'groups', (
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'option_category_ids', (
          select coalesce(jsonb_agg(gco.category_id order by gco.sort_order, gco.id), '[]'::jsonb)
          from public.promotion_group_category_options gco
          where gco.group_id = g.id
        ),
        'option_product_ids', (
...
begin
  ...
  if v_kind in ('build', 'mixed') then
    ...
    loop
      ...
      if jsonb_typeof(v_grp->'option_product_ids') = 'array' and jsonb_array_length(v_grp->'option_product_ids') > 0 then
        v_option_count := 0;
        ...
        if v_option_count < v_min then
          raise exception 'Un grupo no tiene suficientes productos especificos para su minimo';
        end if;
      elsif jsonb_typeof(v_grp->'option_category_ids') = 'array' and jsonb_array_length(v_grp->'option_category_ids') > 0 then
        v_option_count := 0;
        for v_cat_opt in select * from jsonb_array_elements(v_grp->'option_category_ids')
        loop
          v_cat := (v_cat_opt #>> '{}')::bigint;

          perform 1 from public.categories where id = v_cat and restaurant_id = v_rid;
          if not found then raise exception 'Una categoria de opciones no pertenece a tu restaurante'; end if;

          insert into public.promotion_group_category_options (group_id, category_id, sort_order)
          values (v_group_id, v_cat, v_option_count)
        ...
        if (
          ...
          join public.promotion_group_category_options gco on gco.category_id = p.category_id
          where gco.group_id = v_group_id
            and p.restaurant_id = v_rid
            and p.status_id = 1
        ) < v_min then
          raise exception 'Un grupo no tiene suficientes productos en sus categorias para su minimo';
        end if;
      end if;
    end loop;
  end if;
