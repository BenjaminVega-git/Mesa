...
begin
  if exists (
    ...
    alter table public.products rename column scan_code to codigo;
  end if;
end $$;
...
drop index if exists public.products_restaurant_scan_code_unique;

comment on column public.products.codigo is
  'Codigo leido por pistola laser/lector de barras para asociar y agregar productos.';

create unique index if not exists products_restaurant_codigo_unique
  on public.products (restaurant_id, lower(codigo))
